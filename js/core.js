  ...
  categories: [
    {
      id: 'cat-1',
      name: 'Electronics',
      description: 'Devices and digital equipment',
      status: 'Active',
    },
  ...
  customers: [
    {
      ...
      email: 'finance@apexrobotics.com',
      status: 'Active',
    },
    {
      id: 'cus-2',
      ...
      email: 'office@orionworkspace.com',
      status: 'Active',
    },
  ],
  products: [
...
export function read() {
  try {
    return JSON.parse(localStorage.getItem('stockflowData')) || structuredClone(seed);
  } catch {
    return structuredClone(seed);
  }
}
export function write(data) {
  localStorage.setItem('stockflowData', JSON.stringify(data));
}
export function status(text) {
  const kind = /out|cancel|overdue/i.test(text)
    ? 'danger'
    : /low|pending|draft|partial/i.test(text)
      ? 'warning'
      : /sale|purchase|adjust/i.test(text)
        ? 'info'
        : 'success';
...
export function modal(title, content, actions = '') {
  $('#modal-root').innerHTML =
    `<div class="modal-backdrop"><section class="modal"><header class="modal-header"><b>${title}</b><button class="close" data-close-modal>×</button></header><div class="modal-body">${content}</div><footer class="modal-footer">${actions}</footer></section></div>`;
...
const pages = [
  ...
  ['low-stock', 'Low Stock', 'low-stock.html'],
  ['stock-in', 'Stock In', 'stock-in.html'],
  ['stock-out', 'Stock Out', 'stock-out.html'],
  ['adjustments', 'Adjustments', 'adjustments.html'],
  ['movements', 'Stock Movements', 'movements.html'],
...
export function bindSearch(selector, callback) {
  $(selector).addEventListener('input', (event) => callback(event.target.value.toLowerCase()));
}
export function empty(message, button = '') {
  return `<div class="empty"><div>⌕</div>${message}${button ? `<br><br>${button}` : ''}</div>`;
}
